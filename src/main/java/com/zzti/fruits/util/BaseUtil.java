...
//  }
     ...
     * @param str
     * @return
     * 提高查询效率，限制查询范围，暂时默认3个月的交易 --Add By Ms 2017年3月9日21:16:50
     * 获得指定日期的偏移日期
     * 其中MONTH起始参数为0
     * 此方法仅作固定三个月前日期查询
     */
	public static String getOffChangeDate(String flag) throws IOException {	
		
		String fileName = "SysParam.properties";
		Properties properties = new Properties();
		properties.load(BaseUtil.class.getClassLoader().getResourceAsStream(fileName));
		String monthNumber = properties.getProperty("MONTH");// 配置文件里面的查询时间段：3个月
		int month =0;
		if("12".equals(flag.trim())){
			month = -12;
		}else{
...
//			month = -3;
     ...
     * @param str
     * @return
     * 提高查询效率，限制查询范 -- 2017年8月29日20:16:50
     * 获得指定日期的偏移日期
     * 其中DAY起始参数为0
     * 
     */
	public static String getOffChangeTime(String flag) throws IOException{
		
		String fileName ="SysParam.properties";
		Properties pro = new Properties();
		pro.load(BaseUtil.class.getClassLoader().getResourceAsStream(fileName));
		
		String daysNumber = pro.getProperty("DAYS");
		int days = Integer.parseInt(daysNumber);
...
//		String str = getOffChangeDate(MchtContants.MONTH_12);
//		
...
//	}
    ...
    /**
	 * @param str
	 * @return 获取配置文件中的泰隆宝--营业外本金收入账户
	 */
	public static String getTlbCrAcct() throws IOException {

		String fileName = "SysParam.properties";
		Properties properties = new Properties();
		properties.load(BaseUtil.class.getClassLoader().getResourceAsStream(
				fileName));
		String acctNo = properties.getProperty("TLB_CREDIT_ACCT_NO");
		return acctNo;
	}
	
	/**
	 * @param str
	 * @return 获取配置文件中的泰隆宝--营业外利息收入账户
	 */
	public static String getTlbInterAcct() throws IOException {

		String fileName = "SysParam.properties";
		Properties properties = new Properties();
		properties.load(BaseUtil.class.getClassLoader().getResourceAsStream(
				fileName));
		String acctNo = properties.getProperty("TLB_INTER_ACCT_NO");
		return acctNo;
	}
	
	public static String strToBDStr(String num){
        if(StringUtils.isBlank(num)){
            num = "0";
        }
    ...
    public static String getMaxId(String maxId,int len) {
